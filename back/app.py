...
def ask_question():
    ...
    if not schema:
        return jsonify({"error": "Unable to fetch schema from database."}), 500

    generated_query = get_gemini_response(question, dialect, database, schema)
    
    if "ERROR" in generated_query.upper():
        return jsonify({"error": "Invalid query generated.", "query": generated_query})
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if query_type in ["INSERT", "DELETE", "UPDATE"]:
                conn.commit()
                return jsonify({"query": query, "result": {"message": f"{query_type} query executed successfully."}})
            elif query_type == "SELECT" or "JOIN" in query.upper():
                rows = cursor.fetchall()
                columns = [column[0] for column in cursor.description]
                return jsonify({"query": query, "result": {"columns": columns, "rows": [list(row) for row in rows]}})
            else:
                return jsonify({"query": query, "result": {"message": f"{query_type} query executed."}})
    except pyodbc.Error as e:
        return jsonify({"error": str(e), "query": generated_query}), 500
    finally:
        conn.close()
...
def display():
    ...
    question = data.get("question")
    dialect = data.get("dialect", "SQL Server")
    database = data.get("database", "test_db")  # fallback if not provided
    server = data.get("server")  # optional for execution
    schema = {}

    # Optional: fetch schema if server & db are given (for more accurate generation)
    ...
    return jsonify({"sql": cleaned_query})
